...
---------------------------------------------------------------------
SELECT  SC.custid, SC.companyname, SC.contactname, SC.address, 
SC.city, SC.country,  SC.phone FROM SALES.Customers AS SC
WHERE  SC.country = N'BRAZIL'

---------------------------------------------------------------------
-- Task 2
-- 
-- Write a SELECT statement that will return
-- the custid, companyname, contactname, address,
-- city, country, and phone columns from the Sales.Customers table. 
-- Filter the results to include only customers
-- from the countries Brazil, UK, and USA.
--
...
-- 
-- Write a SELECT statement that will return --
--the custid, companyname, contactname, address, city, country,
-- and phone columns from the Sales.Customers table. 
-- Filter the results to include only 
--the customers with a contact name starting with the letter A.
--
...
-- Task 5
-- 
-- Write a T-SQL statement 
--to retrieve customers from the Sales.Customers table 
--that do not have matching orders in the Sales.Orders table. 


-- Matching customers with orders
-- is based on a comparison between the customer’s custid value
-- and the order’s custid value. 

-- Retrieve the custid and companyname columns
...
SELECT SC.custid, SC.companyname FROM SALES.Customers SC
LEFT JOIN SALES.Orders SO ON
SC.custid = SO.custid
