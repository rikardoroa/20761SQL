...
SELECT CURRENT_TIMESTAMP AS CURRENDATETIME
SELECT CAST (CURRENT_TIMESTAMP AS DATE) AS CURRENTDATE
SELECT CAST (CURRENT_TIMESTAMP AS TIME)  AS CURRENTTIME
SELECT DATEPART( YEAR,CURRENT_TIMESTAMP) AS CURRENTYEAR
SELECT DATEPART( MONTH,CURRENT_TIMESTAMP) AS CURRENTMONTH
SELECT DATEPART( DAY,CURRENT_TIMESTAMP) AS CURRENTDAY
SELECT DATENAME (WEEK, CURRENT_TIMESTAMP) AS CURRENTWEEK
...
-- Task 2
--  
-- Write December 11, 2015, 
---as a column with a data type of date. -
--Use the different possibilities inside 
---the T-SQL language (cast, convert, specific function, etc.) and use the alias somedate.
---------------------------------------------------------------------

SELECT  CONCAT (CAST (DATENAME (MONTH,'20151211') AS varchar) ,'  ', '11,',' ', '2015') AS CURRENT_DAY
SELECT  CONCAT (DATENAME (MONTH, DATEFROMPARTS (2015,12,11)) ,'  ', '11,',' ', '2015') AS CURRENT_DAY
---------------------------------------------------------------------
-- Task 3
-- 
-- Write a SELECT statement to return columns that contain:
--  Three months from the current date and time. 
---Use the alias threemonths.
--  Number of days between
--- the current date and 
---the first column (threemonths). Use the alias diffdays.
--  Number of weeks between 
---April 4, 1992, and September 16, 2011. Use the alias diffweeks.
...
---------------------------------------------------------------------
SELECT DATEADD (MONTH ,3,(CAST (CURRENT_TIMESTAMP AS DATE))) AS THEEMONTHS
SELECT DATEDIFF (DAY, (CAST (CURRENT_TIMESTAMP AS DATE)),  DATEADD (MONTH ,3,(CAST (CURRENT_TIMESTAMP AS DATE)))) AS DIFFDAYS
SELECT  DATEDIFF (WEEK, DATEFROMPARTS(1992,4,4), DATEFROMPARTS (2011,9,16)) AS DIFFWEEKS
SELECT DATEADD ( DAY , 1,EOMONTH (CURRENT_TIMESTAMP,-1)) AS FIRSTDAY
---------------------------------------------------------------------
-- Task 4
-- 
...
-- 
-- Write a SELECT statement against the Sales.Somedates
--- table and retrieve the isitdate column. 
---Add a new column named converteddate
-- with a new date data type value based on the column isitdate.
--- If the column isitdate 
----cannot be converted to
--- a date data type for a specific row, then return a NULL. 
SELECT ISITDATE,
TRY_CONVERT ( DATE, ISITDATE,112)
